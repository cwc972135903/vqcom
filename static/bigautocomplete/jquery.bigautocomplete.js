(function($){
    function isBigMatch(key,data){
        return (key==null ||key=="" ||data.title.toLocaleLowerCase().indexOf(key.toLocaleLowerCase())>=0
		||(data.ext&&data.ext.toLocaleLowerCase().indexOf(key.toLocaleLowerCase())>=0))
    }
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			...
			$("#bigAutocompleteContent").delegate("tr", "click", function() {
				...
				if($("#bigAutocompleteContent").css("display") != "none" && callback_ && $.isFunction(callback_)){
                    if($(currentInputText).data('last_value')!=$(this).data("jsonData")) {
                        $(currentInputText).data('last_value',$(this).data("jsonData"))
		...
		this.autocomplete = function(param){
			...
			$this.keydown(function(event) {
				...
				case 40://向下键
					...
					if($nextSiblingTr.length > 0){//有下一行时（不是最后一行）
                        ...
                        $(node).data("jsonData",$nextSiblingTr.data("jsonData"));
			...
			if(this.config.click)
                $this.parent('div').click(function (event) {
                var node = event.currentTarget;
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						for(var i=0;i<data.length;i++){
							if( !k || isBigMatch(keyword_,data[i])){
								data_.push(data[i]);
							}
						}
				...
				if(k == 13){
					...
					if($bigAutocompleteContent.css("display") != "none"){
                        ...
                        //     if($(node).data("config").data&&$(node).data('last_value')!=config.data[o]) {
                        //         $(node).data('last_value',config.data[o]);
